...
namespace Catalog.BLL.Services.Impl
{
        ...
        : IStreetService
    {
        ...
        public IEnumerable<StreetDTO> GetStreets(int pageNumber)
        {
            ...
            var mapper = 
                new MapperConfiguration(
                    cfg => cfg.CreateMap<Item, StreetDTO>()
                    ).CreateMapper();
            var streetsDto = 
                mapper
                    .Map<IEnumerable<Item>, List<StreetDTO>>(
                        streetsEntities);
            return streetsDto;
        }
        ...
        public void AddStreet(StreetDTO street)
        {
            ...
            var mapper = new MapperConfiguration(cfg => cfg.CreateMap<StreetDTO, Item>()).CreateMapper();
            var streetEntity = mapper.Map<StreetDTO, Item>(street);
